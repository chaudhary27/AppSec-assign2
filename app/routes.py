...
def logout():
    ...
    updateLog = TimeLog.query.filter_by(
        uname=currUser, logout_time=None).first()
...
def spellcheck():
    ...
    if form.validate_on_submit():
        ...
        with open('userinput.txt', 'w') as file:
            ...
            file.close()
        textoutput = subprocess.run(['./a.out', 'userinput.txt', 'wordlist.txt'],
                                    stdout=subprocess.PIPE, check=True, universal_newlines=True)
        textmisspell = textoutput.stdout.replace("\n", ", ")[:-2]
        if textmisspell == "":
            textmisspell = "No words were misspelled."

        entry = Spell(uname=currUser, query_text=inputtext,
                      query_results=textmisspell)
